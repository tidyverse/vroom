...
of [`vroom()`](https://vroom.r-lib.org/dev/reference/vroom.md).
...
file, e.g. with `usethis::edit_r_environ()`. For versions of R where the
Altrep framework is unavailable (R \< 3.5.0) they are automatically
turned off and the variables have no effect. The variables can take one
