...
- [`problems()`](https://vroom.r-lib.org/reference/problems.md) works
  normally for vroom-produced objects, even if readr is attached
  ([\#534](https://github.com/tidyverse/vroom/issues/534),
  [\#554](https://github.com/tidyverse/vroom/issues/554)).

- [`problems()`](https://vroom.r-lib.org/reference/problems.md) are no
  longer corrupted if the offending data frame is partially
  materialized, e.g. by viewing a subset, before calling
  [`problems()`](https://vroom.r-lib.org/reference/problems.md)
...
- [`str()`](https://rdrr.io/r/utils/str.html) now works in a colorized
  ...
  with [`col_big_integer()`](https://vroom.r-lib.org/reference/cols.md)
...
- [`vroom::problems()`](https://vroom.r-lib.org/reference/problems.md)
...
- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) reads more
  reliably from filepaths containing non-ascii characters, in a
  non-UTF-8 locale
...
- Fixed recursive garbage collection error that could occur during
  [`vroom_write()`](https://vroom.r-lib.org/reference/vroom_write.md)
  when
  [`output_column()`](https://vroom.r-lib.org/reference/output_column.md)
  generates an ALTREP vector
  ([\#389](https://github.com/tidyverse/vroom/issues/389)).

- [`vroom_progress()`](https://vroom.r-lib.org/reference/vroom_progress.md)
  uses
...
- `col_factor(levels = NULL)` honors the `na` strings of
  [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) and its own
  `include_na` argument, as described in the docs, and now reproduces
  the behaviour of readr’s first edition parser
...
- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) no longer
  issues a spurious warning when the parsing needs to be restarted due
  to the presence of embedded newlines
...
- [`vroom_fwf()`](https://vroom.r-lib.org/reference/vroom_fwf.md) no
  longer omits the last line if it does *not* end with a newline
...
- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) now supports
  files with only carriage return newlines (`\r`).
  ([\#360](https://github.com/tidyverse/vroom/issues/360),
  <https://github.com/tidyverse/readr/issues/1236>)

- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) now parses
  single digit datetimes more consistently as readr has done
  (<https://github.com/tidyverse/readr/issues/1276>)

- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) now parses
  `Inf` values as doubles
  (<https://github.com/tidyverse/readr/issues/1283>)

- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) now parses
  `NaN` values as doubles
...
- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) now works
  around specifying a `\n` as the delimiter
  ([\#365](https://github.com/tidyverse/vroom/issues/365),
  <https://github.com/tidyverse/dplyr/issues/5977>)

- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) no longer
  crashes if given a `col_name` and `col_type` both less than the number
  of columns (<https://github.com/tidyverse/readr/issues/1271>)

- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) no longer
  hangs if given an empty value for `locale(grouping_mark=)`
...
- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) now respects
  the `quote` parameter properly in the first two lines of the file
...
- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) now always
  includes the last row when guessing
...
- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) now supports
  inputs with unnamed column types that are less than the number of
  columns ([\#296](https://github.com/tidyverse/vroom/issues/296))

- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) now outputs
  the correct column names even in the presence of skipped columns
  ([\#293](https://github.com/tidyverse/vroom/issues/293),
...
- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) and
  [`vroom_write()`](https://vroom.r-lib.org/reference/vroom_write.md)
...
- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md),
  [`vroom_fwf()`](https://vroom.r-lib.org/reference/vroom_fwf.md) and
...
- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) now supports
  skipping comments and blank lines within data, not just at the start
  of the file ([\#294](https://github.com/tidyverse/vroom/issues/294),
  [\#302](https://github.com/tidyverse/vroom/issues/302))

- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) now uses the
  tzdb package when parsing date-times
  ([@DavisVaughan](https://github.com/DavisVaughan),
  [\#273](https://github.com/tidyverse/vroom/issues/273))

- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) now emits a
  warning of class `vroom_parse_issue` if there are non-fatal parsing
  issues.

- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) now emits a
  warning of class `vroom_mismatched_column_name` if the user supplies a
  column type that does not match the name of a read column
...
- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) now correctly
  skips lines with only one quote if the format doesn’t use quoting
...
- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) now outputs a
  tibble with the expected number of columns and types based on
  `col_types` and `col_names` even if the file is empty
  ([\#297](https://github.com/tidyverse/vroom/issues/297)).

- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) no longer
  mis-indexes files read from connections with windows line endings when
  the two line endings falls on separate sides of the read buffer
  ([\#331](https://github.com/tidyverse/vroom/issues/331))

- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) no longer
  crashes if `n_max = 0` and `col_names` is a character
  ([\#316](https://github.com/tidyverse/vroom/issues/316))

- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) now preserves
  the spec attribute when vroom and readr are both loaded
  ([\#303](https://github.com/tidyverse/vroom/issues/303))

- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) now allows
  specifying column names in `col_types` that have been repaired
  ([\#311](https://github.com/tidyverse/vroom/issues/311))

- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) no longer
  inadvertently calls `.name_repair` functions twice
  ([\#310](https://github.com/tidyverse/vroom/issues/310)).

- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) is now more
  robust to quoting issues when tracking the CSV state
  ([\#301](https://github.com/tidyverse/vroom/issues/301))

- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) now registers
  the S3 class with
  [`methods::setOldClass()`](https://rdrr.io/r/methods/setOldClass.html)
  (r-dbi/DBI#345)

- [`col_datetime()`](https://vroom.r-lib.org/reference/cols.md) now
  supports ‘%s’ format, which represents decimal seconds since the Unix
  epoch.
...
- [`vroom_fwf()`](https://vroom.r-lib.org/reference/vroom_fwf.md) gains
  a `skip_empty_rows` argument to skip empty lines
  (<https://github.com/tidyverse/readr/issues/1211>)

- [`vroom_fwf()`](https://vroom.r-lib.org/reference/vroom_fwf.md) now
  respects `n_max`, as intended
...
- [`problems()`](https://vroom.r-lib.org/reference/problems.md) now
  prints the correct row number of parse errors
  ([\#326](https://github.com/tidyverse/vroom/issues/326))

- [`problems()`](https://vroom.r-lib.org/reference/problems.md) now
  throws a more informative error if called on a readr object
  ([\#308](https://github.com/tidyverse/vroom/issues/308)).

- [`problems()`](https://vroom.r-lib.org/reference/problems.md) now
  de-duplicates identical problems
...
- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) now signals an
  R error for invalid date and datetime formats, instead of crashing the
  session ([\#220](https://github.com/tidyverse/vroom/issues/220)).
...
- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) now works
  without error with readr versions of col specs
...
- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) now handles
  cases when `id` is set and a column in skipped
  ([\#237](https://github.com/tidyverse/vroom/issues/237))

- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) now supports
  column selections when there are some empty column names
  ([\#238](https://github.com/tidyverse/vroom/issues/238))

- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) argument
  `n_max` now works properly for files with windows newlines and no
  final newline ([\#244](https://github.com/tidyverse/vroom/issues/244))
...
- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) now writes the
  column names if given an input with no rows
  ([\#213](https://github.com/tidyverse/vroom/issues/213))

- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) columns now
  support indexing with NA values
  ([\#201](https://github.com/tidyverse/vroom/issues/201))

- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) no longer
  truncates the last value in a file if the file contains windows
  newlines but no final newline
  ([\#219](https://github.com/tidyverse/vroom/issues/219)).

- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) now works when
  the `na` argument is encoded in non ASCII or UTF-8 locales *and* the
  file encoding is not the same as the native encoding
  ([\#233](https://github.com/tidyverse/vroom/issues/233)).

- [`vroom_fwf()`](https://vroom.r-lib.org/reference/vroom_fwf.md) now
  verifies that the positions are valid, namely that the begin value is
  always less than the previous end
...
- [`vroom_altrep_opts()`](https://vroom.r-lib.org/reference/vroom_altrep_opts.md)
...
- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) now supports
  reading Big Integer values with the `bit64` package. Use
  [`col_big_integer()`](https://vroom.r-lib.org/reference/cols.md) or
  the “I” shortcut to read a column as big integers.
  ([\#198](https://github.com/tidyverse/vroom/issues/198))

- [`cols()`](https://vroom.r-lib.org/reference/cols.md) gains a `.delim`
  argument and [`vroom()`](https://vroom.r-lib.org/reference/vroom.md)
  now uses it as the delimiter if it is provided
  ([\#192](https://github.com/tidyverse/vroom/issues/192))

- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) now supports
  reading from [`stdin()`](https://rdrr.io/r/base/showConnections.html)
...
- [`fwf_empty()`](https://vroom.r-lib.org/reference/vroom_fwf.md) now
  uses the `skip` parameter, as intended.

- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) can now read
  single line files without a terminal newline
  ([\#173](https://github.com/tidyverse/vroom/issues/173)).

- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) can now select
  the id column if provided
  ([\#110](https://github.com/tidyverse/vroom/issues/110)).

- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) now correctly
  copies string data for factor levels
  ([\#184](https://github.com/tidyverse/vroom/issues/184))

- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) no longer
  crashes when files have trailing fields, windows newlines and the file
  is not newline or null terminated.

- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) now includes a
  spec object with the `col_types` class, as intended.

- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) now better
  handles floating point values with very large exponents
  ([\#164](https://github.com/tidyverse/vroom/issues/164)).

- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) now uses
  better heuristics to guess the delimiter and now throws an error if a
  delimiter cannot be guessed
...
- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) now has an
  improved error message when a file does not exist
...
- [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) no longer
  overflows when reading files with more than 2B entries
  ([@wlattner](https://github.com/wlattner),
  [\#183](https://github.com/tidyverse/vroom/issues/183)).

- [`vroom_fwf()`](https://vroom.r-lib.org/reference/vroom_fwf.md) is now
  more robust if not all lines are the expected length
  ([\#78](https://github.com/tidyverse/vroom/issues/78))

- [`vroom_fwf()`](https://vroom.r-lib.org/reference/vroom_fwf.md) and
  [`fwf_empty()`](https://vroom.r-lib.org/reference/vroom_fwf.md) now
  support passing `Inf` to `guess_max()`.

- [`vroom_str()`](https://vroom.r-lib.org/reference/vroom_str.md) now
  works with S4 objects.

- [`vroom_fwf()`](https://vroom.r-lib.org/reference/vroom_fwf.md) now
  handles files with dos newlines properly.
...
- [`vroom_write()`](https://vroom.r-lib.org/reference/vroom_write.md) no
  longer crashes non-deterministically when used on Altrep vectors.
...
- Move example files for
  [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) out of
  `\dontshow{}`.
...
- Default message output from
  [`vroom()`](https://vroom.r-lib.org/reference/vroom.md) now uses
  `Rows` and `Cols` ([@meta00](https://github.com/meta00),
