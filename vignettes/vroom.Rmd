...
`vroom()` decompresses, indexes and writes the decompressed file to the temp directory in a single stream.
...
If you are reading a zip file that contains multiple files, you can use a wrapper function as follows: 
...
You can read all the column with the same type, for example as a character. 

```{r}
vroom(vroom_example("mtcars.csv"), col_types = list(.default = "c"))
```
...
Use `vroom_write()` to write delimited files, the default delimiter is tab, to write TSV files. Writing to TSV by default has the following benefits:
  ...
  - More easily and efficiently ingested by Unix command line tools such as
    `cut`, `perl` and `awk`.
...
However you can easily set `delim` to `delim = ','` to write CSV files, which
are common as inputs to GUI spreadsheet tools like Excel or Google Sheets.
...
vroom supports reading and writing to the C-level `stdin` and `stdout` of the R
process by using `stdin()` and `stdout()`. E.g. from a shell prompt you can
pipe to and from vroom directly.
...
Note this interpretation of `stdin()` and `stdout()` differs from that used by
other R functions, however we believe it better matches most user's expectations.
