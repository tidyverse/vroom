...
file <- "https://raw.githubusercontent.com/Rdatatable/data.table/master/vignettes/flights14.csv"
vroom(file)
```
...
vroom(file, col_select = c(year, carrier, air_time))
```
- A numeric vector of column indexes, e.g. `c(1, 2, 5)`
```{r}
vroom(file, col_select = c(1, 6, 9))
```
...
- Or rename columns
```{r}
vroom(file, col_select = list(company = carrier, everything()))
```
