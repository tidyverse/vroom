...
Often the names of columns in the original dataset are not ideal to work with. `vroom()` uses the same .name_repair argument as tibble, so you can use one of the default name repair strategies or provide a custom function A great approach is to use the [`janitor::make_clean_names()`](https://www.rdocumentation.org/packages/janitor/versions/1.2.0/topics/make_clean_names) function as the input.
