...
read_benchmark <- function(file, desc) {
  vroom::vroom(file, col_types = c("ccccdddd")) %>%
    filter(op != "setup") %>%
    mutate(
      altrep = case_when(
        grepl("^vroom_no_altrep", reading_package) ~ "none",
        grepl("^vroom_", reading_package) ~ "normal",
        TRUE ~ NA_character_
        ),
...
plot_benchmark <- function(data, title) {
  ...
  data %>%
    filter(reading_package != "read.delim") %>%
    ggplot() +
...
taxi_writing %>%
  select(-size, -op, -rows, -cols, -type, -altrep, -label) %>%
  mutate_if(is.numeric, pretty_sec) %>%
  spread(package, time) %>%
