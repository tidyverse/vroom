...
The dataset used in the following benchmark is from Freedom of Information Law (FOIL)
...
[bench/taxi_benchmark.R](https://github.com/r-lib/vroom/blob/master/inst/bench/taxi_benchmark.R).
...
tm_df %>%
  ...
  geom_bar(aes(x = package, y = time, fill = op, group = package), stat = "identity") +
    ...
    coord_flip() +
    labs(title = "Real time to analyze trip fare data", x = NULL, y = NULL, fill = NULL)
```
...
tm_df <- vroom::vroom(system.file("bench", "taxi-times.tsv", package = "vroom")) %>%
  filter(type == "real") %>%
  mutate(
    package = fct_reorder(sub("_", "\n", package), time, sum),
    op = factor(op, desc)
...
times <- tm_df %>%
  ...
  tally(wt = time) %>%
  spread(op, n) %>%
  mutate(total = sum(setup, read, print, head, tail, sample, filter, aggregate))

times %>%
  arrange(desc(total)) %>%
  mutate_if(is.numeric, prettyunits::pretty_sec) %>%
...
si <- vroom::vroom(system.file("bench", "sessioninfo.tsv", package = "vroom"), col_types = cols())
