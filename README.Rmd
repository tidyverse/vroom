...
<!-- badges: start -->
[![Travis build status](https://travis-ci.org/jimhester/vroom?branch=master)](https://travis-ci.org/jimhester/vroom)
<!-- badges: end -->
...
tbl <- tm %>% dplyr::filter(type == "real", op == "read", package != "vroom_dplyr") %>%
  ...
  transmute(
    ...
    "throughput" = prettyunits::pretty_bytes(file.size("~/data/trip_fare_1.tsv") / time))
```

The fastest delimited reader for R, **`r dplyr::filter(tbl, package == "vroom") %>% pull("throughput") %>% paste0("/sec")`**.

But that's impossible! How can it be [so fast](https://raw.githack.com/jimhester/vroom/master/bench/benchmarks.html)?
...
you use. It also has no (current) support for quoted fields, comments,
whitespace trimming and other niceties.
