...
tm_df <- map_dfr(times, function(x) {
  ops <- c("read", "print", "head", "tail", "sample", "fitler")
  tibble::tibble(
...
library(gt)
tm_df %>% dplyr::filter(op == "read", type == "real") %>% select(package, time) %>% gt::gt() %>% fmt_number(vars(time)) %>% tab_header("Real time to read the file", "(base and dplyr both use `readidx`, so are equivalent)")
```

```{r, echo = FALSE, warning = FALSE}
tm_df %>% group_by(package) %>% dplyr::filter(type == "real") %>% count(wt = time, name = "time") %>% gt::gt() %>% fmt_number(vars(time)) %>% tab_header("Total time for all operations")
```
