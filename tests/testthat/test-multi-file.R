...
test_that("vroom adds the id column from the filename for multiple connections", {
  ...
  for (i in seq_along(splits)) {
    ...
    vroom_write(
      splits[[i]],
      file.path(dir, paste0("mtcars_", names(splits)[[i]], ".tsv.gz")),
      delim = "\t"
    )
  }
...
test_that("vroom errors if numbers of columns are inconsistent", {
  ...
  skip(
    "libvroom does not error on inconsistent column counts across files (pads with NA instead)"
  )
...
test_that("vroom works if some files contain no data, regardless of order (#430)", {
  ...
  vroom_write_lines(
    c("A,B", "1,2"),
    file.path(destdir, "header_and_one_row.csv")
  )
...
test_that("vroom works for indxes that span file boundries (#383)", {
  x <- vroom(
    c(vroom_example("mtcars.csv"), vroom_example("mtcars.csv")),
    col_types = list()
  )
