...
vroom <- function(file, fields, types, desc) {
...
`vroom (full altrep)` <- function(file, fields, types, desc) {
  bench::workout(description = desc,
    {
      ({library(vroom); library(dplyr)})
      x <- vroom_fwf(file, fields, col_types = types, altrep_opts = TRUE)
      print(x)
      a <- head(x)
      b <- tail(x)
      c <- sample_n(x, 100)
...
`vroom (no altrep)` <- function(file, fields, types, desc) {
  bench::workout(description = desc,
    {
      ({library(vroom); library(dplyr)})
...
readr <- function(file, fields, types, desc) {
  bench::workout(description = desc,
    {
...
read.delim <- function(file, fields, types, desc) {
  bench::workout(description = desc,
    {
      ({})
...
times <- list(
  vroom_dplyr = callr::r(vroom, list(file, fields, types, desc)),
  `vroom (full altrep)_dplyr` = callr::r(`vroom (full altrep)`, list(file, fields, types, desc)),
  `vroom (no altrep)_dplyr` = callr::r(`vroom (no altrep)`, list(file, fields, types, desc)),
  readr = callr::r(readr, list(file, fields, types, desc)),
  read.delim = callr::r(read.delim, list(file, fields, types, desc))
)
...
data <- vroom_fwf(file, fields, col_types = types)
...
vroom::vroom_write(
  tm_df,
  here::here("inst", "bench", "fwf-times.tsv"),
  delim = "\t")
