# from http://www.oecd.org/pisa/pisaproducts/INT_STU12_DEC03.zip
file <- "~/data/INT_STU12_DEC03.txt"
file <- "~/data/INT.txt"
desc <- c("setup", "read", "print", "head", "tail", "sample", "filter", "aggregate")

library(vroom)
line_len <- max(nchar(readLines(file, n = 10)))
fields <- fwf_empty(file)
widths <- fields$end - fields$begin
widths[length(widths)] <- line_len - fields$begin[length(widths)]

vroom <- function(file, desc, fields, widths) {
  bench::workout(description = desc,
    {
      z <- vroom_fwf(file, fields, altrep = TRUE)
      y <- readr::read_fwf(file, fields)
      x <- read.fwf(file, widths)
      print(x)
      a <- head(x)
      b <- tail(x)
