...
// Skip leading comment lines and blank/whitespace-only lines in the data.
// Returns offset past all leading comment and blank lines.
// A comment line starts with the comment string (at column 0) and ends at newline.
// Blank lines contain only whitespace (spaces/tabs) before a newline.
static size_t skip_leading_comment_lines(const char* data, size_t size, const std::string& comment) {
  if (size == 0) {
    return 0;
  }
  ...
  while (offset < size) {
    ...
    if (offset >= size) {
      // End of data (trailing whitespace only) — return start of this blank line
      return line_start;
    }

    // Check for blank line (newline immediately after whitespace)
    if (data[offset] == '\n') {
      offset++;
      continue;
    }
    ...
    // Not a blank or comment line — return start of this line (before whitespace)
    return line_start;
  }
  return offset;
}
