...
Result<bool> CsvReader::start_streaming() {
  ...
  // Create the chunk queue.  Use num_chunks as the buffer size so producer
  // threads never block on push().  With a small max_buffered (e.g. 4),
  // out-of-order chunk completion can cause all pool threads to block on
  // push() while the consumer blocks on pop() waiting for a specific chunk
  // that hasn't been parsed yet — a deadlock when the thread that would
  // parse it is itself blocked on push().
  impl_->streaming_queue = std::make_unique<ParsedChunkQueue>(num_chunks, num_chunks);
  ...
  for (size_t chunk_idx = 0; chunk_idx < num_chunks; ++chunk_idx) {
    ...
    pool.detach_task([queue_ptr, data, size, chunk_idx, start_offset, end_offset, start_inside,
      ...
      try {
        ...
        // Ensure the chunk is always pushed even on failure, so the
        // consumer never blocks forever waiting for a missing chunk.
        std::vector<std::unique_ptr<ArrowColumnBuilder>> empty;
        queue_ptr->push(chunk_idx, std::move(empty));
      }
    });
